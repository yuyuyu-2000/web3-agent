...
def make_monitor_tools() -> list[StructuredTool]:
    def create_monitor_rule(rule_type: str, address: str | None = None,
        ...
        rule = monitor_store().create_rule(
            user_id=current_monitor_user(), rule_type=rule_type, address=address,
            min_amount=Decimal(str(min_amount)) if min_amount is not None else None,
            min_amount_usd=Decimal(str(min_amount_usd)) if min_amount_usd is not None else None,
            chain=chain, token=token, notification_channel=notification_channel,
        )
        return _json({"status": "success", "rule": asdict(rule)})
