...
def test_budget_trims_low_priority_context_first() -> None:
    result = builder().build(
        ...
        [
            ...
            ContextPart("current_request", [HumanMessage(content="current request")], 2, True),
            ContextPart("memory", [SystemMessage(content="useful old memory words")], 6),
            ContextPart(
                "recent_history",
                [HumanMessage(content="recent one"), HumanMessage(content="recent two")],
                7,
                newest_first=True,
            ),
            ContextPart("summary", [SystemMessage(content="very old summary words here")], 8),
        ],
    )
...
def test_protected_context_is_never_silently_removed() -> None:
    with pytest.raises(ContextBudgetError, match="protected context"):
        builder(4).build(
            "reviewer",
            [ContextPart("system", [SystemMessage(content="too many protected words")], 1, True)],
        )
...
def test_router_keeps_only_eight_recent_non_tool_messages() -> None:
    ...
    result = context_builder.router(
        system_prompt="router rules", current_request="current", history=history,
        tool_names="none",
    )
...
def test_budget_respects_context_window_minus_reserved_output() -> None:
    result = ContextBuilder("test", model_context_window=100, max_input_tokens=95, reserved_output_tokens=20)
    assert result.max_input_tokens == 80


def test_tool_request_and_result_are_trimmed_atomically() -> None:
    context_builder = builder(8)
    tool_request = AIMessage(content="", tool_calls=[{"id": "call-1", "name": "query", "args": {}}])
    ...
    result = context_builder.build(
        ...
        [
            ...
            ContextPart("recent_history", [tool_request, tool_result], 7, newest_first=True),
        ],
    )
