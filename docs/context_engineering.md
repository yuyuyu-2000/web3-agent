...
- 没有统一的 token 预算器；
- 没有针对静态 Schema 的动态检索；
- 长期记忆仍需显式总结；选择既支持 `memory_key`，也支持 gated 自动语义召回。
...
`memory_key` 仍是最高优先级。未传 key 时，只有确定性 gate 判断消息明显依赖跨线程
历史，才会在当前 `user_id` 范围内执行 embedding + pgvector/cosine 检索。候选经过
最低相似度阈值后最多注入 3 条；同一 thread 的相关追问复用 checkpoint 中的召回结果。
当前事实查询直接跳过，召回基础设施异常也只会降级为不注入。

当前仍没有记忆冲突检测、过期策略或每 N 轮自动总结。
