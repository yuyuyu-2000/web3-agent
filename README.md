...
| 编排 | `agent/graph.py` | **唯一编排入口**：每轮先生成结构化计划，再逐步骤执行；包含 `Planner`、步骤选择、执行器、`ToolNode`、工具调用预算和 `compile(checkpointer=...)`。简单问题生成单步骤计划。 |
...
- `POST /chat`：JSON 体 `{"thread_id": "<会话 id>", "message": "<用户消息>"}`，在 `thread_id` 维度恢复 checkpoint。
